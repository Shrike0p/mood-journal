        ...
        <div className="mt-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {entries.map((entry, idx) => (
                    ...
                    >
                        <div className="text-sm text-gray-700 dark:text-gray-600">{new Date(entry.date).toLocaleDateString()}</div>
                        <div className="text-2xl my-1">
                            {moodEmojis[entry.mood] || ''}
                            <span className="ml-2 text-lg font-medium text-gray-800 dark:text-gray-800">{entry.mood}</span>
                        </div>
                        <div className="text-gray-700 dark:text-gray-800 mb-2 line-clamp-2">{entry.note}</div>
                        {entry.weather && (
                            <div className="text-xs text-gray-700 dark:text-gray-800/60">
